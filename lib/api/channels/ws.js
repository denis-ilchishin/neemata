...
class WsChannel extends Channel {
  ...
  bind() {
    ...
    this.application.sandbox.application.wss = {
      ...
      // rooms: {
      //   roo: (roomId) => rooms.get(roomId),
      //   join: (roomId, client) => rooms.join(roomId, client),
      //   leave: (roomId, client) => rooms.leave(roomId, client),
      // },
    }
    ...
    this.fastify.get(
      ...
      (conn, req) => {
        ...
        //
        ;(async () => {
          const auth = await this.handleAuth(false, { req })
  ...
  async handleMessage({ conn, message, auth, req }) {
    ...
    if (message.type === 'room') {
      ...
      try {
        const apiModule = this.application.api.get(module, version, Protocol.Ws)
