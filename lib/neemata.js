...
const baseDir = process.cwd() // TODO: make configurable via cli args
...
class Neemata extends EventEmitter {
    ...
    return new Promise((resolve) => {
      for (const worker of this.workers) {
        ...
        worker.on('exit', () => {
          ...
          if (exited.length === this.workers.length) resolve()
        })
      }
    })
  }
  ...
  createWorker(data = {}) {
    ...
    const env = {
      ...process.env,
      NODE_PATH: `${process.env.NODE_PATH}:${resolve(baseDir, 'node_modules')}`,
    }
